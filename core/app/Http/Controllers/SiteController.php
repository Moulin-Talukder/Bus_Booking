...
class SiteController extends Controller
{
    ...
    public function getTicketPrice(Request $request){
        ...
        $getPrice = $ticketPrice->prices()->where('source_destination', json_encode($sdArray))->first();
    ...
    public function bookTicket(Request $request,$id){
        ...
        // $var = BookedTicket::where('trip_id', $id)->where('date_of_journey', Carbon::parse($request->date)->format('Y-m-d'))->whereIn('status',[1,2])->where('pickup_point', $request->pickup_point)->where('dropping_point', $request->dropping_point)->get('seats');
        // $booked_ticket  = BookedTicket::where('trip_id', $id)->where('date_of_journey', Carbon::parse($request->date)->format('Y-m-d'))->whereIn('status',[1,2])->where('pickup_point', $request->pickup_point)->where('dropping_point', $request->dropping_point)->WhereIn( rtrim($request->seats, ","),$var)->get();
        
        $var = BookedTicket::where('trip_id', $id)
        ...
        ->pluck('seats')
        ->all(); // Extract and convert the seats into an array
    
        $seatsArray = explode(',', rtrim($request->seats, ',')); // Convert $request->seats into an array
        ...
        $booked_ticket = BookedTicket::where('trip_id', $id)
            ...
            ->where('pickup_point', $request->pickup_point)
            ->where('dropping_point', $request->dropping_point)
            ->whereIn('seats', $seatsArray) // Use the $seatsArray in the whereIn clause
            ->get();
    ...
    public function ticketSearch(Request $request)
    {
        ...
        return view($this->activeTemplate.'ticket', compact('pageTitle' ,'fleetType', 'trips','routes', 'schedules', 'emptyMessage', 'layout','dayOff'));
    }
